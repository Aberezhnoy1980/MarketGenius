...
requests_url = {
    'history_secs': 'http://iss.moex.com/iss/history/engines/{engine}/markets/{market}/boards/{board}/securities.json?date={date}',
    'test': 'https://iss.moex.com/iss/history/engines/stock/markets/shares/securities/MOEX.csv?iss.only=history'
    }
...
class MicexAuth:
    ...
    def auth(self):
        ...
        opener.addheaders = [('Authorization',
                              f'Basic {base64.b64encode((self.config.user + ":" + self.config.password).encode()).decode()}')]
...
class MicexISSClient:
    ...
    def get_history_securities(self, engine, market, board, date):
        ...
        """
        url = requests_url['history_secs'].format(engine=engine,
                                          market=market,
                                          board=board,
                                          date=date)
